...
export function calculateProjectCostEstimate(moduleData = {}) {
  const breakdown = {};

  if (moduleData.robotic_systems?.robots) {
    breakdown.robotic_systems = calculateRoboticsHardwareCost(moduleData.robotic_systems.robots);
  }

  if (moduleData.conveyance?.segments) {
    breakdown.conveyance = calculateConveyanceHardwareCost(moduleData.conveyance.segments);
  }

  if (moduleData.storage?.zones) {
    breakdown.storage = calculateStorageInfrastructureCost(moduleData.storage.zones);
  }

  if (moduleData.controls?.panels) {
    breakdown.controls = calculateControlsElectricalCost(moduleData.controls.panels);
  }

  if (moduleData.software?.applications) {
    breakdown.software = calculateSoftwareSystemsCost(moduleData.software.applications);
  }

  if (moduleData.implementation?.services) {
    breakdown.implementation = calculateImplementationServicesCost(moduleData.implementation.services);
  }
